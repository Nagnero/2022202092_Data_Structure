...
TermsBSTNode* TermsBST::search(string target_date) {
	...
	while (1) {
		...
		int res = target_date.compare(curNode->getTermDate());
...
bool TermsBST::_delete(string target_date) {
	TermsBSTNode* destNode = search(target_date);
