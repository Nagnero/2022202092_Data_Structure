...
// delete
void TermsLIST::name_delete(string name, string target_date, char term) {
	TermsListNode* curTLNode = this->getHead();
	...
	// return 1 when empty
	int check_empty = curBST->_delete(target_date);
...
void TermsLIST::date_delete(NameBST* nb, string termDate) {
	...
	while (curTLNode) {
		...
		if (!check_empty) this->delete_success = true;
		...
		if (!curBST->getRoot()) {
			...
			// more than one node
				...
				// delNode is Head
				if (this->getHead() == delNode) {
					this->setHead(delNode->getNext());
				}
				// delNode is not Head
				else {
					TermsListNode* prevNode = this->getHead();
